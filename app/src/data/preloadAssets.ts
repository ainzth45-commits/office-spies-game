...
// หน้าโลโก้ (BootScreen) ใช้ progress จากที่นี่กั้นไม่ให้เข้าเกมก่อนรูปครบ
let started = false;
...
function loadNext() {
  ...
  const img = new Image();
  img.decoding = "async";
  // นับต่อไม่ว่าสำเร็จหรือพัง (รูปพังมี onError fallback ในแต่ละหน้าอยู่แล้ว) — progress ต้องถึง 100% เสมอ ไม่ค้าง
  const done = () => {
    loadedCount += 1;
