...
// ค่า default (สเกล 2026-07-04): reward 10 ลด 1 ทุก 5 วิ (พื้น 1) · โทษ 3 ภายใน 30 วิ · เกิน = 6
describe("quizRewardAt", () => {
  it("ตอบทันที ได้เต็ม", () => {
    expect(quizRewardAt(0, defaultConfig)).toBe(10);
  });
  it("ก่อนครบ step แรก ยังเต็ม", () => {
    expect(quizRewardAt(4.9, defaultConfig)).toBe(10);
  });
  it("ครบ step ลดทีละ 1", () => {
    expect(quizRewardAt(5, defaultConfig)).toBe(9);
    expect(quizRewardAt(17, defaultConfig)).toBe(7);
  });
  it("ไม่ต่ำกว่าพื้น", () => {
    expect(quizRewardAt(50, defaultConfig)).toBe(1);
...
describe("quizPenaltyAt", () => {
  it("ภายในเกณฑ์เวลา = ขั้นปกติ", () => {
    ...
    expect(quizPenaltyAt(30, defaultConfig)).toBe(3);
  });
  it("เกินเกณฑ์ = ขั้นแรง และไม่บานต่อ", () => {
    expect(quizPenaltyAt(30.1, defaultConfig)).toBe(6);
